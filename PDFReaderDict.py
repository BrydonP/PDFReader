...
for idx, file in enumerate(fileList):
    try:## Some files have bad EOF marker
    ...
    print("Processing: \t" + str(dataDict['CRC ALERT CASESPOT TICKET NUMBER'].value))
    for idx, data in enumerate(form_fields):
        if(dataDict[form_fields[idx]] is not None and form_fields[idx] is not None): # Check for Empty Field
